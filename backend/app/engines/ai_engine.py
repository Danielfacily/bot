"""AI Engine — classifica probabilidade de sucesso baseado em features."""
...
class AIEngine:
    ...
    def train(self, rows: list[dict]) -> dict:
        ...
        df = pd.DataFrame(rows).dropna(subset=[f for f in FEATURES if f in pd.DataFrame(rows).columns] + ["success"])
        ...
        pipeline.fit(df[available], df["success"].astype(int))
        self.model_path.parent.mkdir(parents=True, exist_ok=True)
        joblib.dump(pipeline, self.model_path)
        self.model = pipeline
        return {"trained": True, "rows": len(df), "features": available}
