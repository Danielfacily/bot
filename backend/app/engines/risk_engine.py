"""Risk Engine — avalia condições de risco antes da entrada."""
...
class RiskEngine:
    ...
    def evaluate(
        ...
        # Consecutive losses block
        blocked, block_reason = self.check_consecutive_losses(db, mode)
        if blocked and not manual_override:
            return RiskDecision(False, block_reason)
        ...
        # Max exposure check
        exposure_ok = self.check_max_exposure(db, balance, mode)
        if not exposure_ok and not manual_override:
            return RiskDecision(False, "Exposição total acima de 80% do saldo.", leverage=leverage)
    ...
    def check_consecutive_losses(self, db: Session, mode: str, max_losses: int = 3) -> tuple[bool, str]:
        """Retorna (blocked, reason) se houver sequência de perdas."""
        recent = (
            db.query(Trade)
        ...
        if len(recent) < max_losses:
            return False, ""
        if all((float(t.pnl or 0) < 0) for t in recent):
            return True, f"Bloqueado após {max_losses} perdas consecutivas. Aguarde análise."
        return False, ""

    def check_max_exposure(self, db: Session, balance: float, mode: str, max_pct: float = 80.0) -> bool:
        ...
        if balance <= 0:
            return True
        risk_pct = margin_used / balance * 100
        return risk_pct <= max_pct
