...
class AIExitManager:
    async def manage(self, db: Session, order_manager: OrderManager) -> None:
        ...
        for trade in open_trades:
            try:
                ...
                log_event(
                    ...
                    "warning",
                    "ai_exit",
                    "Posição fechada pela gestão IA de saída.",
                    {
                        "symbol": trade.symbol,
                        "side": trade.side,
                ...
                log_event(db, "error", "ai_exit", "Falha na gestão IA de saída.", {"symbol": trade.symbol, "error": str(exc)})

    async def evaluate_trade(self, trade: Trade, order_manager: OrderManager, config: dict) -> ExitDecision:
        candles = await order_manager.client.klines(trade.symbol, "1m", 80)
        enriched = enrich_indicators(candles).dropna()
        if len(enriched) < 25:
            return self.hold(trade, 0, 0, float(trade.exit_price or trade.entry_price), {"data": "insufficient"})
        ...
        adverse_score, continuation_score, details = self.score_market(trade, enriched)
        details.update({"peak_roi": round(peak_roi, 4), "current_roi": round(current_roi, 4)})

        giveback = peak_roi - current_roi
        min_profit_to_protect = float(config.get("ai_exit_min_profit_roi", 4.0))
        hard_adverse = adverse_score >= 78 and adverse_score > continuation_score + 18
        profit_fade = peak_roi >= min_profit_to_protect and giveback >= max(2.5, peak_roi * 0.35) and adverse_score >= 55
        ...
        return self.hold(trade, adverse_score, continuation_score, exit_price, details)

    def score_market(self, trade: Trade, enriched) -> tuple[int, int, dict]:
        latest = enriched.iloc[-1]
        previous = enriched.iloc[-2]
        ...
        if trade.side == "LONG":
            if close < float(latest["vwap"]):
                adverse += 18
                reasons.append("perdeu_vwap")
            else:
                continuation += 14
            if close < float(latest["ema_9"]):
                adverse += 14
                reasons.append("abaixo_ema9")
            if ema_bear:
                adverse += 16
                reasons.append("ema9_abaixo_ema21")
            elif ema_bull:
                continuation += 12
            if close < open_price and force_candle:
                adverse += 16
                reasons.append("candle_vendedor_forca")
            if volume_spike and close < open_price:
                adverse += 14
                reasons.append("volume_vendedor")
            if rejection and upper_wick / candle_range > 0.42:
                adverse += 14
                reasons.append("rejeicao_topo")
            if rsi_value >= 72 and close < open_price:
                adverse += 10
                reasons.append("rsi_extremo_reversao")
            if recent_high_fail:
                adverse += 10
                reasons.append("falha_renovar_maxima")
            if macd_falling:
                adverse += 8
            ...
            if close > float(latest["vwap"]):
                adverse += 18
                reasons.append("recuperou_vwap")
            else:
                continuation += 14
            if close > float(latest["ema_9"]):
                adverse += 14
                reasons.append("acima_ema9")
            if ema_bull:
                adverse += 16
                reasons.append("ema9_acima_ema21")
            elif ema_bear:
                continuation += 12
            if close > open_price and force_candle:
                adverse += 16
                reasons.append("candle_comprador_forca")
            if volume_spike and close > open_price:
                adverse += 14
                reasons.append("volume_comprador")
            if rejection and lower_wick / candle_range > 0.42:
                adverse += 14
                reasons.append("rejeicao_fundo")
            if rsi_value <= 28 and close > open_price:
                adverse += 10
                reasons.append("rsi_extremo_reversao")
            if recent_low_fail:
                adverse += 10
                reasons.append("falha_renovar_minima")
            if macd_rising:
                adverse += 8
    ...
    def hold(self, trade: Trade, adverse_score: int, continuation_score: int, exit_price: float, details: dict) -> ExitDecision:
        return ExitDecision("hold", "hold", adverse_score, continuation_score, exit_price, details)
